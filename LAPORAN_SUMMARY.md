...
```plantuml
@startuml
skinparam backgroundColor #252422
skinparam defaultFontColor #fffcf2
skinparam ArrowColor #eb5e28
skinparam ActivityBorderColor #ccc5b9
skinparam ActivityBackgroundColor #403d39
skinparam ActivityDiamondBackgroundColor #403d39
skinparam ActivityDiamondBorderColor #eb5e28

start
:Pengguna membuka aplikasi;
:Pilih mode input\n(O/M/P atau Per Minggu);

if (Mode Per Minggu?) then (ya)
  :Input durasi tiap aktivitas\n(dalam minggu);
  :Konversi otomatis ke O, M, P\nO=80%, M=100%, P=130%;
else (tidak)
  :Input O, M, P tiap aktivitas\n(dalam hari);
endif

:Validasi data\n(O ≤ M ≤ P, tidak ada siklus);
...
:Hitung Variance (σ²)\nσ² = ((P - O) / 6)²;
:Bangun DAG dari dependensi aktivitas;
:Forward Pass\n→ Hitung ES dan EF tiap node;
:Backward Pass\n→ Hitung LS dan LF tiap node;
:Hitung Slack = LS - ES;
:Tandai jalur kritis\n(aktivitas dengan Slack = 0);

fork
  :Render Network Diagram\n(node merah = kritis);
fork again
  :Tampilkan Tabel Hasil PERT;
fork again
  :Tampilkan Summary Card\n(durasi + jalur kritis);
end fork

if (User klik node?) then (ya)
  :Tampilkan NodeDetailPanel\n(rumus step-by-step);
else (tidak)
endif
